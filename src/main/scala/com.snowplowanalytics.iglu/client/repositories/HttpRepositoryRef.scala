...
import java.net.{URL, UnknownHostException, MalformedURLException}
...
object HttpRepositoryRef {
  ...
  /**
    ...
    */
  private[client] case class HttpConnection(uri: String,
  ...
  private def stringToUrl(url: String): Validated[URL] =
    (try {
      ...
      case mue: MalformedURLException =>
        "Provided URL string [%s] is malformed: [%s]"
          .format(url, mue.getMessage)
          .failure
...
/**
  ...
  * HTTP.
  */
case class HttpRepositoryRef(override val config: RepositoryRefConfig,
                             uri: String,
  ...
  def lookupSchema(schemaKey: SchemaKey): Validated[Option[JsonNode]] = {
    try {
      for {
        url <- HttpRepositoryRef.stringToUrl(
          s"$uri/schemas/${schemaKey.toPath}")
