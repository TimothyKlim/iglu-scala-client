...
// This project
import repositories.{RepositoryRef, EmbeddedRepositoryRef, HttpRepositoryRef}
...
object Resolver {
  ...
  private val ConfigurationSchema = SchemaCriterion(
  ...
  /**
    ...
    */
  private[client] case class RepoError(errors: Set[ProcessingMessage],
                                       attempts: Int,
  ...
  /**
    ...
    */
  private[client] implicit object RepoErrorSemigroup
      extends Semigroup[RepoError] {
    def append(a: RepoError, b: => RepoError): RepoError =
      RepoError(a.errors |+| b.errors,
  ...
  def parse(config: JsonNode): ValidatedNel[Resolver] = {
    ...
    config
      .verifySchemaAndValidate(ConfigurationSchema, dataOnly = true) match {
      case Success(node) => {
  ...
  private[client] def buildRepositoryRef(
      repositoryConfig: JValue): ValidatedNel[RepositoryRef] = {
  ...
  /**
    ...
    */
  @tailrec
  def traverseRepos(schemaKey: SchemaKey,
  ...
  /**
    ...
    */
  private[client] def prioritizeRepos(schemaKey: SchemaKey,
                                      repositoryRefs: RepositoryRefs) =
    repositoryRefs.sortBy(
      r =>
        (!r.vendorMatched(schemaKey),
         r.classPriority,
         r.config.instancePriority))
  ...
  private[client] def collectErrors(
    ...
    val failures = tried.toList.collect { case (_, Some(error)) => error }.flatMap {
      _.errors.toList
    }
...
case class Resolver(
  ...
  def lookupSchema(schemaKey: SchemaKey,
                   attempts: Int = 3): ValidatedNel[JsonNode] = {
      ...
      case Some(schema) =>
        schema match {
          ...
          case Failure(serverErrors) =>
            ...
            traverseRepos(schemaKey,
                          Resolver.prioritizeRepos(schemaKey, reposForRetry),
                          serverErrors)
        }
      case None =>
        traverseRepos(schemaKey, prioritizeRepos(schemaKey, allRepos), Map())
    }
  ...
  def unsafeLookupSchema(schemaKey: SchemaKey): JsonNode =
    lookupSchema(schemaKey) match {
      ...
      case Failure(err) =>
        throw new RuntimeException(s"Unsafe schema lookup failed: ${err}")
    }
}
