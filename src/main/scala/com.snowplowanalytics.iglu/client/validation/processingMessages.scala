...
object ProcessingMessageMethods {
  ...
  /**
    ...
    */
  def toProcMsg(message: String,
                logLevel: LogLevel = LogLevel.ERROR): ProcessingMessage =
    new ProcessingMessage().setLogLevel(logLevel).setMessage(message)

  def toProcMsg(messages: NonEmptyList[String],
                logLevel: LogLevel): ProcessingMessageNel =
    messages.map(msg => toProcMsg(msg, logLevel))

  def toProcMsgNel(message: String,
                   logLevel: LogLevel = LogLevel.ERROR): ProcessingMessageNel =
