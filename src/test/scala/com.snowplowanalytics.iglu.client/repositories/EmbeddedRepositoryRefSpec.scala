...
class EmbeddedRepositoryRefSpec
  ...
  def e2 = {
    val expected = EmbeddedRepositoryRef(
      config = RepositoryRefConfig("Acme Embedded",
                                   100,
                                   List("uk.co.acme", "de.acme")),
      path = "/acme-embedded-new"
    )
  ...
  def e3 = {
    ...
    SpecHelpers.EmbeddedTest
      ...
      .map(_.map(_.toString)) must beSuccessful(Some(expected))
  }
  ...
  def e5 = {
    ...
    SpecHelpers.EmbeddedTest
      .lookupSchema(schemaKey)
      .leftMap(_.toString) must beFailing(expected.toString)
  }
