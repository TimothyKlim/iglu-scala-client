...
class SelfDescValidationSpec extends Specification with ValidationMatchers {
  ...
  def e3 =
    invalidJson.validate(false).leftMap(_.map(_.toString)) must beFailing(
      invalidExpected)

  val expectedKey = SchemaKey("com.snowplowanalytics.iglu-test",
                              "stock-item",
                              "jsonschema",
                              "1-0-0")

  val expectedCriterion = SchemaCriterion("com.snowplowanalytics.iglu-test",
                                          "stock-item",
                                          "jsonschema",
                                          1,
                                          0,
  ...
  def e4 =
    validJson.validateAndIdentifySchema(false) must beSuccessful(
      (expectedKey, validJson))
  ...
  def e6 =
    invalidJson
      ...
      .leftMap(_.map(_.toString)) must beFailing(invalidExpected)

  def e7 =
    validJson
      .verifySchemaAndValidate(expectedCriterion, false) must beSuccessful(
      validJson)

  def e8 =
    validJson
      .verifySchemaAndValidate(expectedCriterion, true) must beSuccessful(
      validJson.get("data"))

  val incorrectKey = SchemaCriterion("com.snowplowanalytics.iglu-test",
                                     "stock-item",
                                     "jsonschema",
                                     2,
                                     0,
