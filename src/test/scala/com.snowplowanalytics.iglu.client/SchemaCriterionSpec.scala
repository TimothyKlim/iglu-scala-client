...
class SchemaCriterionSpec
  ...
  def e1 = {
    ...
    SchemaCriterion("com.snowplowanalytics.snowplow",
                    "payload_data",
                    "jsonschema",
                    1).matches(actual) must beFalse
  }
  ...
  def e2 =
    "SPEC NAME" || "Criterion" | "SchemaVer" | "EXPECTED OUTPUT" |
      ...
      "Correct model, lower revision, higher addition" !! ((2,
                                                            Some(3),
                                                            Some(4))) ! "2-0-9" ! true |> {
  ...
  def e3 = {
    val criterion = SchemaCriterion.parse(
