...
object ResolverSpec {
  ...
  object Repos {
    ...
    private val embedRef: (String, Int) => EmbeddedRepositoryRef =
      (prefix, priority) =>
        EmbeddedRepositoryRef(
...
class ResolverSpec
  ...
  def e1 = {
    val resolver = Resolver(cacheSize = 0,
                            SpecHelpers.IgluCentral,
                            Repos.one,
                            Repos.two,
                            Repos.three)
    val schemaKey = SchemaKey("de.acompany.snowplow",
                              "mobile_context",
                              "jsonschema",
                              "1-0-0")
  ...
  def e2 = {
    ...
    val expected =
      Resolver(cacheSize = 500, SpecHelpers.IgluCentral, Repos.three)
  ...
  def e3 = {
    ...
    val expected = NonEmptyList(
      notFoundError(
        ...
        List("Iglu Test Embedded [embedded]",
  ...
  def e4 = {
    ...
    val schemaKey = SchemaKey("com.snowplowanalytics.iglu-test",
                              "corrupted_schema",
                              "jsonschema",
                              "1-0-0")
    val expected = NonEmptyList(
      notFoundError(
        ...
        List("Iglu Client Embedded [embedded]",
             "Iglu Test Embedded [embedded]")
      ),
  ...
  def e5 = {
    ...
    val expected = NonEmptyList(
      ...
      "array must have at least one element".toProcessingMessage
        .put("domain", "syntax")
        .put("schema",
             asJsonNode(parse("""{"loadingURI":"#","pointer":""}""")))
  ...
  def e7 = {
    val schemaKey = SchemaKey("com.snowplowanalytics.iglu-test",
                              ...
                              "jsonschema",
                              "1-0-0")
    val timeout = new ProcessingMessage()
      .setLogLevel(LogLevel.ERROR)
      .setMessage("Timeout exception")
    ...
    resolver.lookupSchema(schemaKey) must beFailing.like {
      case error =>
        error.toList.map(_.toString) must contain(
          notFoundError(schemaKey.toString,
                        List("Iglu Client Embedded [embedded]",
                             "Mock Repo [null]")),
          timeout.toString
        )
  ...
  def e8 = {
    val schemaKey = SchemaKey("com.snowplowanalytics.iglu-test",
                              ...
                              "jsonschema",
                              "1-0-0")
    val error1 = new ProcessingMessage()
      .setLogLevel(LogLevel.ERROR)
      .setMessage("Timeout exception")
    val error2 = new ProcessingMessage()
      .setLogLevel(LogLevel.ERROR)
      .setMessage("Network exception")
    val error3 = new ProcessingMessage()
      .setLogLevel(LogLevel.ERROR)
      .setMessage("Unknown exception")
    val error4 = new ProcessingMessage()
      .setLogLevel(LogLevel.ERROR)
      .setMessage("Server segfault")
    ...
    // Stubbing
    httpRep1.lookupSchema(schemaKey) returns error1
      .failure[Option[JsonNode]] thenReturns error2.failure[Option[JsonNode]]
    httpRep2.lookupSchema(schemaKey) returns error3
      .failure[Option[JsonNode]] thenReturns error4.failure[Option[JsonNode]]
